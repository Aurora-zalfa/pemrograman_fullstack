...
/**
 * ========================================================
 * 1. KOMPONEN PROTECTED ROUTE (SKELETON KEAMANAN SPRINT 9)
 * ========================================================
 * Memastikan user tidak bisa nembak URL /dashboard jika belum login.
 */
const ProtectedRoute = ({ children, isAuthenticated, userRole, allowedRoles }) => {
  // Jika token tidak valid / belum login, tendang ke login
  if (!isAuthenticated) {
    return <Navigate to="/login" replace />;
  }

  // Jika halaman butuh role spesifik dan user tidak punya haknya, balikkan ke home
...
/**
 * ========================================================
 * 2. KOMPONEN ROUTER UTAMA (APP) DENGAN PENDISTRIBUSIAN STATE
 * ========================================================
 */
function App() {
  ...
  return (
    <Router>
      <Routes>
        ...
        {/* Jalur Halaman Login -> Passing state login callback */}
        <Route path="/login" element={<Login onLoginSuccess={handleLoginSuccess} />} />
        
        {/* Jalur Dashboard Terproteksi (Protected Route & State Distribusi) */}
        <Route 
          path="/dashboard/*" 
          element={
            <ProtectedRoute 
